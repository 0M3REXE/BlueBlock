  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Recent verification activities and statuses.</p>
      </div>

      <div className="flex flex-wrap gap-4">
        <FilterBadge label="All" active />
        <FilterBadge label="Pending" />
        <FilterBadge label="In Review" />
        <FilterBadge label="Approved" />
        <FilterBadge label="Rejected" />
      </div>
          ...
          <tbody>
            {rows.map(v => (
              <tr key={v.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-gray-700">{v.site_id ? v.site_id.slice(0,8) : '-'}</td>
                <td className="px-3 py-2 text-gray-700">{v.project_id ? v.project_id.slice(0,8) : '-'}</td>
                <td className="px-3 py-2">
                  <StatusBadge status={v.status} />
                </td>
                <td className="px-3 py-2">{v.method || '-'}</td>
                <td className="px-3 py-2 text-center">{v.findings_count ?? 0}</td>
                <td className="px-3 py-2 text-gray-500">{new Date(v.created_at).toLocaleDateString()}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
                <td colSpan={7} className="px-3 py-6 text-center text-gray-500">No verifications found.</td>
              </tr>
            )}
          </tbody>
        </table>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${color}`}>{status}</span>;
}
...
function FilterBadge({ label, active }: { label: string; active?: boolean }) {
  return (
    <button className={`px-3 py-1 rounded-full border text-xs font-medium transition ${active ? 'bg-black text-white border-black' : 'bg-white hover:bg-gray-50'}`}>{label}</button>
  );
}
